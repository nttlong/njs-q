...
var exp_mongo=require("./expr_mongo");
var operators = {
    ">": "$gt",
    ">=": "$gte",
    "<": "$lt",
    ...
    if (fx.name) {
        ...
        if(fx.args){
            ...
            for(var i=0;i<fx.args.length;i++){
                _params.push(get_left(fx.args[i],params));
            }
            var ret={};
            if (fx.type =="FunctionNode"){
                field="$"+field;
...
//     if(expr.substring(0,"expr(".length)=="expr(" &&
//         expr[expr.length-1]==')'){
//         var _expr_ = expr.substring("expr(".length, expr.length - 1);
//         return {
//             $expr: __filter(expr,params)
//         };
//     }
//     else {
//         return __filter(expr, params);
//     }
// }
function filter(expr, params) {
    if ((!(params instanceof Array)) &&
